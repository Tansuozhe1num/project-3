...
int eval_mm_valid(const malloc_impl_t* impl, trace_t* trace, int tracenum) {
  ...
  for (i = 0; i < trace->num_ops; i++) {
    ...
    case REALLOC:  // realloc
      ...
      // TODO(project3): YOUR CODE HERE
      // 1. 校验旧块中的任何数据在 realloc 后是否保持完整
      for (int j = 0; j < oldsize; j++) {
          if ((unsigned char)newp[j] != 0x5A) {
              malloc_error(tracenum, i, "Data corruption detected after realloc! Old data was not preserved.");
              return 0;
          }
      }
      
      // 2. 校验通过后，用新的独特数据 0xA5 覆盖新块，以便下次继续校验
      if (size > 0) {
